...
namespace KoiVM.Runtime.Execution.Internal {
	internal class EHHelper {
		delegate void Throw(Exception ex, string ip, bool rethrow);

		static Throw rethrow;

		static bool BuildExceptionDispatchInfo(Type type) {
			try {
		...
		static bool BuildInternalPreserve(Type type) {
			try {
		...
		static readonly object RethrowKey = new object();
