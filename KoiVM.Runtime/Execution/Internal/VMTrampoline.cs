...
namespace KoiVM.Runtime.Execution.Internal {
	internal static class VMTrampoline {
		delegate RuntimeMethodHandle GetMethodDescriptor(DynamicMethod dm);

		static readonly GetMethodDescriptor getDesc;
		static readonly MethodInfo entryStubNormal;
		static readonly MethodInfo entryStubTyped;

		static readonly Hashtable trampolines = new Hashtable();

		static VMTrampoline() {
			foreach (var method in typeof(SupremeVM.SupremeVM).GetMethods(BindingFlags.Static | BindingFlags.NonPublic)) {
