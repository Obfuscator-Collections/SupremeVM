...
namespace KoiVM.Runtime.Execution {
	internal static class VMDispatcher {
		...
		static unsafe string GetIP(VMContext ctx) {
		...
		static uint rand_state = (uint)Environment.TickCount;

		static unsafe string StackWalk(VMContext ctx) {
			uint ip = (uint)(ctx.Registers[Constants.REG_IP].U8 - (ulong)ctx.Instance.Data.KoiSection);
			uint bp = ctx.Registers[Constants.REG_BP].U4;
		...
		static void HandleEH(VMContext ctx, ref ExecutionState state) {
			...
			switch (ehState.CurrentProcess) {
				case EHState.EHProcess.Searching: {
					if (ehState.CurrentFrame != null) {
						// Return from filter
						bool filterResult = ctx.Registers[Constants.REG_R1].U1 != 0;
						if (filterResult) {
